class CreateGame {
  ...
  createWalls() {
    let tableauMurs = []
    while (tableauMurs.length < walls) {
      let alea = {
        x: this.randomNb(board),
        y: this.randomNb(board)
      };
      let isUnique = true;
  ...
  popWeapons(tableauMurs) {
    ...
    while (tableauArmes.length < nbArmes) {
      let isUnique = true;
      let weapon = new weapons(this.randomNb(weapons.armes.length));
      let alea = {
        x: this.randomNb(board),
        y: this.randomNb(board),
        weapon: weapon
